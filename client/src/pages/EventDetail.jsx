...
export default function EventDetail() {
  ...
  useEffect(() => {
    const loadEvent = async () => {
      setLoading(true);
      try {
        // fetch event (your axiosInstance uses same base style as other calls)
        const res = await axios.get(`/events/${id}`, { withCredentials: true });
        if (res && res.data) {
          // your backend returns event object (res.data or res.data.event)
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow p-6 md:flex md:gap-6">
          ...
          <div className="mt-4 md:mt-0 md:w-1/2">
            ...
            <div className="mt-6">
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <button
                  onClick={() => navigate("/events")}
                  className="bg-transparent border px-4 py-2 rounded"
                >
                  Back
                </button>
              </div>
